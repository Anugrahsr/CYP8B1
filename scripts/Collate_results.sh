...
for i in `ls -1 *.txt`
do
pval=`grep "^Like" $i|awk '{print $6}'|sed 's/\*\*\.//g'`
kval1=`grep "Relaxation/intensification" $i|awk '{print $6}'|head -1`
kval2=`grep "Relaxation/intensification" $i|awk '{print $6}'|tail -1`
test=`grep "_test_ set:" $i|awk '{print $9}'|sed 's/\`//g'`
ref=`grep "_reference_ set:" $i|sed 's/\, /\,/g'|awk '{print $9}'|sed 's/\`//g'`
refcount=`echo $ref|tr "," "\n"|wc -l`
other=`grep "unclassified (nuisance) set:" $i|sed 's/\, /\,/g'|awk '{print $10}'|sed 's/\`//g'`
tree=`echo $i|cut -f 2 -d '.'|sed 's/tree_//g'`
comb=`echo $i|cut -f 4 -d '.'|sed 's/comb_//g'`
